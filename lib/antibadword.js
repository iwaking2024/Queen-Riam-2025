...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    if (!match) {
        return sock.sendMessage(chatId, {
            text: `*ANTIBADWORD SETUP*\n\n*.antibadword on*\nActivar antimalas palabras\n\n*.antibadword set <action>*\nEstablecer acción: delete/kick/warn\n\n*.antibadword off*\nDesactiva antibadword en este grupo`
        });
    }
    ...
    if (match === 'on') {
        ...
        if (existingConfig?.enabled) {
            return sock.sendMessage(chatId, { text: '*AntiBadword ya está habilitado para este grupo*' });
        }
        await setAntiBadword(chatId, 'on', 'delete');
        return sock.sendMessage(chatId, { text: '*Se ha habilitado AntiBadword. Utilice .antibadword set <acción> para personalizar la acción*' });
    }

    if (match === 'off') {
        const config = await getAntiBadword(chatId, 'on');
        if (!config?.enabled) {
            return sock.sendMessage(chatId, { text: '*AntiBadword ya está deshabilitado para este grupo*' });
        }
        await removeAntiBadword(chatId);
        return sock.sendMessage(chatId, { text: '*AntiBadword ha sido deshabilitado para este grupo*' });
    }

    if (match.startsWith('set')) {
        const action = match.split(' ')[1];
        if (!action || !['delete', 'kick', 'warn'].includes(action)) {
            return sock.sendMessage(chatId, { text: '*Acción no válida. Elegir: delete, kick, o warn*' });
        }
        await setAntiBadword(chatId, 'on', action);
        return sock.sendMessage(chatId, { text: `*Acción AntiBadword establecida en: ${action}*` });
    }

    return sock.sendMessage(chatId, { text: '*Comando no válido. Utilice .antibadword para ver el uso*' });
}
...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
   ...
   // console.log('Bad word detected in:', userMessage);
    ...
    switch (antiBadwordConfig.action) {
        case 'delete':
            await sock.sendMessage(chatId, {
                text: `*@${senderId.split('@')[0]} aquí no se permiten malas palabras*`,
                mentions: [senderId]
            });
            break;
        ...
        case 'kick':
            try {
                ...
                await sock.sendMessage(chatId, {
                    text: `*@${senderId.split('@')[0]} ha sido expulsado por usar malas palabras*`,
                    mentions: [senderId]
                });
            } catch (error) {
        ...
        case 'warn':
            ...
            if (warningCount >= 3) {
                try {
                    ...
                    await sock.sendMessage(chatId, {
                        text: `*@${senderId.split('@')[0]} ha sido expulsado después de 3 advertencias*`,
                        mentions: [senderId]
                    });
                } catch (error) {
                ...
                await sock.sendMessage(chatId, {
                    text: `*@${senderId.split('@')[0]} advertencia ${warningCount}/3 por usar malas palabras*`,
                    mentions: [senderId]
                });
            }
            break;
