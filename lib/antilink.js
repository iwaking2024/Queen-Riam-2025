		...
		switch (action) {
			case 'delete':
				await sock.sendMessage(jid, { 
					text: `\`\`\`@${sender.split('@')[0]} No se permiten enlaces aquí\`\`\``,
					mentions: [sender] 
				});
				break;
			...
			case 'warn':
				...
				if (warningCount >= WARN_COUNT) {
					...
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} Ha sido pateado después ${WARN_COUNT} warnings\`\`\``,
						mentions: [sender]
					});
				} else {
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} advertencia ${warningCount}/${WARN_COUNT} para enviar enlaces\`\`\``,
						mentions: [sender]
					});
				}
				break;
